...
\begin{proof}
   If we can make the four coins form a larger square with side length $n > 1$ in a few jumps. Then if the initial side length is $1/n$ we can use the same steps of jump to form a square of side length $1$.
    ...
    It means that with the converse steps, We can form a smaller square with side length $1/n < 1$ with initial side length $1$, which requiring that the coordinates of coins are not integer.
     ...
     \textbf{
     \begin{itemize}
     \item 1.Everything is reversible.
     \item 2.If we can form a larger square, we can also form a a smaller square.
     \item 3.This is impossible since you always stay on the grid
     \item P.S. Hexagon can be also proofed as above but not pentagon.
     \end{itemize}}
\end{proof}
...
%Define $A\\B$ is a set of elements in A but not in B.
...
%$|A\bigcap B\bigcap C| =|A\bigcap B\bigcap D|=|A\bigcap C\bigcap D| =  1$
...
\end{proof}
 ...
 \textbf{Definition}: 
 	 ...
 	 %1.Define $|A|$ is the number of elements in A.
