...
\begin{proof}\end{proof}
	...
	\end{equation}
		 ...
		 $(2)$ now becomes\\
		 \begin{equation}
		  \sum\limits_{k=1}^{n-1}(-1)^{k+1}A_{n-1,k} + \left\vert{A_n}\right\vert +  \sum\limits_{k=1}^{n-1}(-1)^{k}B_{n-1,k}
		 \end{equation}

     Since $A_{n-1,k+1} + B_{n-1,k} = A_{n,k}$, it finally beconmes: $\sum\limits_{k=1}^{n}(-1)^{k+1}A_{n,k}$
     \begin{equation}
     \end{equation}
