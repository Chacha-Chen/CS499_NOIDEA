...
\begin{solution}
  Let $(a_1,\dots,a_n) \in \N_0^n$. There is a multigraph with this score if and only if regarding its reordered sequence in ascending order--$(b_1,\dots,b_n) \in \N_0^n$ with $b_1 \le b_2 \le \dots d_n$--1) $\sum_{i = 1}^n b_i$ is even and 2) $\sum_{i=1}^{n-1} b_i \ge b_n$.\\
\end{solution}
...
\textbf{Hypothesis:} Any sequcence $\mathbf{a^{'}} = (a_1^{'},\dots,a_{n-1}^{'})$ is a multigraph score satisfying both criterions, then it has corresponding multigraph with score $\mathbf{a}$ of size $n$. \\
\textbf{Base case:} When $n=2$, $\mathbf{a_1} = (a_1, a_2)$ meeting the conditons obviously has $a_1 = a_2$, then it is a multigraph score.\\
When $n=3$, let $\mathbf{a} = (a_1, a_2, a_3)$ meeting the conditons be in nodecreasing order. If $a_1 + a_2= a_3$, we can construct a multigraph with two nodes have $a_1$ and $a_2$ edges connected to third node. Else, we recursively reduce $a_1$ and $a_2$ by $1$ until $a_1 + a_2 = a_3$, we can construct the multigraph according above graph by adding edges between nodes $a_1$ and $a_2$.\\
\textbf{Induction proof:} For any sequence $\mathbf{a} = (a_1, a_2, \dots a_n)$ satisfying both criterions, we sort it to get an ascending order sequence $\mathbf{b} = (b_1, b_2, \dots b_n)$ with $v_i$ denoting the node with degree $b_i$.\\
Assuming $v_1$ has all edges connected to $v_n$, remove $v_1$ and we get new sequence $\mathbf{b^{'}} = (b_2,b_3, \dots,b_{n-1}, b_n - b_1)$\\
...
\end{exercise}
\begin{solution}
  Let $(a_1,\dots,a_n) \in \R_0^n$. There is a weighted graph with this score if and only if corresponding sorted ascending sequence $(b_1,\dots,b_n)$ meets $\sum_{i=1}^{n-1} b_i \ge b_n$.
\end{solution}
...
Besides, for any sequence $\mathbf{a} = (a_1,\dots,a_n) \in \R_0^n$ with $\sum_{i = 1}^{n-1}a_i \ge a_n$, we use induction theorem to prove the theorem.\\
\textbf{Hypothesis}: Any sequence $\mathbf{a_{'}} = (a_1^{'},\dots,a_{n-1}^{'}) \in \R_0^n$ which is weighted graph score has a corresponding sequence $\mathbf{a} = (a_1,\dots,a_n) \in \R_0^n$ meeting the criterion is a weighted graph score.\\
