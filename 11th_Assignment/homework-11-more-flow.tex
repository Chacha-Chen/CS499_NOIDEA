...
\begin{proof}
Since $G$ is $d$-regular, we have $|\Gamma(L_k)| = |R| = |L|$. Also according to Hall's Therorem, we can find a matching with size $|L| - \delta(L) = |L|$, which is a perfect matching.
\end{proof}
...
\item Transform the vertex capacity network to edge capacity network, then we can use Ford-Fulkerson Algorithm to find the flow of network is $\binom{n}{i}$, which means there are $\binom{n}{i}$ content with the two conditions.
\end{enumerate}
\end{proof}
