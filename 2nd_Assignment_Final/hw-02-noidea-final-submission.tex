...
\begin{exercise}
  ...
  \textbf{solution:} Yes, $\{$ ($x_{1}$,$x_{2}$,...,$x_{n}$) $\in$ $N^{n}_{0}$ $\mid$ $x_{1}$+$x_{2}$+...+$x_{n}$ =k-1, k$\in$ $N^{n}_{0}$ $\}$ is an antichain of size k. And since $k$ can be arbitrarily large, it has an arbitrarily large antichain.
\end{enumerate}
\end{exercise}
...
\begin{solution}
Consider an infinite antichain $A \subseteq \nn$, since $A$ is an infinite subset of $\nn$, $A$ must contain an infinite chain according to \textbf{2.3}. Then we get a contradiction.\\
