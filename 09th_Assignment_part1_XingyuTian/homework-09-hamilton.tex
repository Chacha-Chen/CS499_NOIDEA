...
<<<<<<< HEAD:9th_Assignment_part1_XingyuTian/homework-09-hamilton.tex
\begin{solution}
...
\begin{proof}
\end{proof}
>>>>>>> 359fb447132eee1076befcb7eafae04961a474d2:9th_Assignment/homework-09-hamilton.tex
...
\begin{enumerate}
	%\item If a vertex has both incoming edges and outgoing edges, we can explore it along this path. When exploring the next vertex,  find a next outgoing edge and repeat this loop until having exploring all vertices.
	%\item Since every vertex is connected to all other vertices, it seems clearly for every graph like this we can find a Hamilton path. To prove it, now we prove that there don't exist situations finding no Hamilton path.

	%\item Suppose there exist a graph having no directed Hamilton paths.

	%Choose a vertex as a start randomly, if it has an incoming edge, we choose the () as a start
	...
	%\item If there exist $(v_i,v_{i+1})$ that $u$ is connected with them by $(v_i,u)$,$(u,v_{i+1})$.

	%Clearly we can insert u into the path:$(v_1,v_2...v_i,u,v_{i+1},...,v_k)$.

	%\item Else if all edges are from $v_i$ to $u$,or from $u$ to $v_i$, add $u$ as the tail or head of the path.Then we get $(v_1,...v_k,u)$ or $(u,v_1,...v_k)$.
...
\begin{solution}
  Assuming we have a path $u_1, u_2, \dots, u_k$ which is not a Hamilton path and there are no edges starting with $u_k$.\\
  Then we have another vertex $v$ with edge $(v,u_k)$.\\
  If we have edge $(v,u_1)$, we can construct larger path $v, u_1, u_2, \dots u_k$.\\
  If there is no edge $(v,u_1)$, since we have edges $(u_1,v)$ and $(v，u_k)$, we can find a pair of vertexs $(u_i,u_{i+1})$ with edges $(u_i,v)$ and $(v,u_{i+1})$ where $1 \le i < k$. Then can construct a larger path $u_1, \dots u_i, v, u_{i+1} \dots u_k$.\\
  We recursively process above steps until we have all vertexs in the path, and we get a Hamilton path.\\
\end{solution}
...
If there is a vertex ,we write 1 to explore it and then write 1 to return.

Take the following tree as an example.
\begin{center}
\includegraphics[width=0.5\textwidth]{figures/Figure9-5-1.pdf}\\
%{\small $T_1,T_2...T_9$}
\end{center}
...
However, in a tree if the leftchild tree of a node is the same as the leftchild tree of the same node, they are isomorphism.BUt they have different 0/1-sequences.
